...
3. A Firestore outbox occurrence keyed by `eventKey` stores only the canonical payload hash, subject key/version, delivery state, lease timestamps, task name, attempt count, fixed failure code, and a seven-day TTL after terminal state. Pending records do not expire before the Cloud Tasks retry window closes. A transaction permits one writer; identical retries resume/return the first outcome, while changed payloads conflict.
4. Queue `analytics-delivery` runs in `asia-south1` at at most 2 dispatches/second and 2 concurrent dispatches. Its task name is a separate HMAC of `eventKey`, and its body contains only a random opaque delivery handle—never uid, subject/event key, event properties, or learner content. The task calls a non-public delivery route with an OIDC token from a dedicated task identity. It attempts at most 8 times over 24 hours with 10-second minimum and 15-minute maximum backoff. The runtime identity can enqueue only on this queue; the task identity can invoke only the delivery service.
...
| Firestore outbox occurrences | Pending through the bounded delivery window; terminal records 7 days by TTL; removed earlier on account deletion | Cloud Run runtime and dedicated task identities only |
...
The user approved Gate G7 as revised on 2026-08-28. SC-700 is `DONE`, and SC-705 is `READY` for a separate provisioning task. This approval does not itself authorize SC-710 implementation, product deployment, or any resource outside this contract.
